...
- **`unifi_setting`: add 8 new sections — `locale`, `global_switch`, `global_nat`, `dashboard`, `traffic_flow`, `ether_lighting`, `netflow`, `ssl_inspection`.** Each follows the same opt-in pattern as the existing sections: configure only the nested block you need, and the provider preserves every other setting untouched. `global_switch` models its full wire surface, including `switch_exclusions` (a MAC-address exclusion list). Three related settings keys — `ipsec`, `usg_geo`, `global_network` — are deferred: `usg_geo`'s fields already live under the existing `usg` section (no separate key), and `ipsec`/`global_network` need a live-controller capture before they can be modeled correctly.
