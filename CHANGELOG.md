...
- **`unifi_setting`: add 4 new sections — `mdns`, `teleport`, `magic_site_to_site_vpn`, `radio_ai`.** Each follows the same opt-in, preserve-by-default pattern as the existing sections. `mdns.mode` is a discriminator: `predefined_services`/`custom_services` are only live when `mode = "custom"` (rejected at plan time otherwise, normalized to empty on read/write when inactive). `radio_ai` splits its channel/width-selection fields into a `UseStateForUnknown`-protected group so the AI radio optimizer's out-of-band changes don't produce spurious diffs when left unconfigured, while still honoring a pinned value on every apply. `magic_site_to_site_vpn` models only `enabled`; any additional controller-managed field is preserved untouched via the existing read-modify-write mechanism.
