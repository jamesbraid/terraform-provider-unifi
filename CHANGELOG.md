...
- **`unifi_setting`: add the `snmp` section (SNMPv2/v3 monitoring configuration).** Exposes `enabled`, `community` (SNMPv2 community string), `enabled_v3`, `username`, and `password` (SNMPv3 passphrase). `community` and `password` are sensitive, write-only-by-convention fields — like the existing `radius.secret` and `mgmt.ssh_password`, the controller never echoes them back, so state preserves the last-configured value and an unset field is omitted from the write payload rather than clobbering whatever the controller already has stored. Note: the underlying SDK struct has no separate SNMPv3 auth/priv-protocol selector or priv passphrase, so this section only exposes what's currently generated.
