...
- **`unifi_setting`: replace the two ad-hoc per-section lifecycle paths with one snapshot-driven, ownership-aware engine — fixing an empty-vs-absent bug, a silent-normalization bug, and a secret-clobber risk along the way.** All 13 sections (`auto_speedtest`, `country`, `doh`, `dpi`, `igmp_snooping`, `ips`, `lcm`, `mgmt`, `network_optimization`, `ntp`, `radius`, `syslog`, `usg`) now decode/overlay through one codec instead of hand-rolled per-section conversions, closing the drift between sections that had grown inconsistent fixes for the same class of bug. The schema is unchanged (attribute names and types are byte-identical, guarded by a schema-equivalence golden) and no state migration is needed. Four behaviors were deliberately corrected as part of the unification, all previously reachable but wrong: **(1)** a remote or configured present-but-empty string/list (e.g. `""` or `[]`) is now preserved as a real value instead of being collapsed to `null` on read; **(2)** malformed remote data — a fractional number where an integer is modeled, a wrong scalar/container type, or a non-string list element — now raises a diagnostic and aborts instead of being silently truncated or dropped; **(3)** a write-only secret (RADIUS `secret`, `mgmt.ssh_password`) is never re-sent from a masked controller read-back (`******`) — an unset config now omits the field from the PUT and preserves whatever the controller already has stored, closing a path that could silently clobber a stored secret with a literal mask string; **(4)** every section now universally preserves unmodeled/advanced fields on update (read-modify-write against a fresh snapshot), not just the handful of sections that happened to do this before. Also: `unifi_setting` import now takes just the site name (e.g. `terraform import unifi_setting.default default`) and hydrates all 13 sections as observed `Computed` values on the first `Read`, producing a clean, no-config re-plan instead of requiring the full block to be reconstructed by hand.
- **`unifi_firewall_policy`: stop sending a stale source/destination selector after switching `matching_target` or `port_matching_type`.** Changing a policy endpoint's match type (e.g. `IP` → `NETWORK`) without also clearing the now-inactive selector (`ips`, `network_ids`, `client_macs`, `web_domains`, `port`, `port_group_id`, `ip_group_id`) left the old value sitting in state and re-sent it on every subsequent apply, because those fields are `Computed` with `UseStateForUnknown` (added for #338 to stop unrelated-field churn) and the conversion code sent whatever was present regardless of which selector the active match type actually owns. The provider now nulls the inactive selector in the plan itself (not just on read/write), and rejects a config that sets a selector the active match type doesn't own with a plan-time error instead of silently dropping it. No state upgrade is required — this PR does not change any attribute's type (the related list→set collection-semantics cleanup is deferred to a follow-up PR; see that PR's entry when it lands).
- **`unifi_firewall_policy` / `unifi_firewall_rule`: reject port values of `0`, above `65535`, or a reversed range (e.g. `443-80`) at plan time instead of failing at apply.** `unifi_firewall_policy`'s `source`/`destination` `port` field previously validated only the string's shape (digits, commas, hyphens), not whether each port or range was semantically valid; `unifi_firewall_rule`'s `src_port`/`dst_port` had no validation at all.
