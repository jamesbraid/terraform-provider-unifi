...
- `matching_target` (String) What to match: `ANY`, `NETWORK`, `CLIENT`, `IP`, `DEVICE`, `MAC`, `WEB` (domains/FQDN), `APP` (DPI applications), or `APP_CATEGORY` (DPI application categories).
...
- `app_category_ids` (Set of Number) DPI application category IDs to match. Used when `matching_target` is `APP_CATEGORY`.
- `app_ids` (Set of Number) DPI application IDs to match (integers, e.g. from the UniFi application list). Used when `matching_target` is `APP`.
