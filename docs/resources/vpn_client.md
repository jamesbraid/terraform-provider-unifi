...
- `private_key_wo` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Write-only equivalent of `private_key` (Terraform 1.11+). Used at apply time but never written to state, so it can be sourced from an ephemeral resource (e.g. a Vault secret). Mutually exclusive with `private_key`; pair with `private_key_wo_version` to rotate it.
- `private_key_wo_version` (Number) Version counter for `private_key_wo`. Terraform cannot compare a write-only value between applies, so a rotation has nothing else to notice; increment this to trigger one.
