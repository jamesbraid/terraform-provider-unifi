...
// THE MAPPING COVERAGE THAT SURVIVED THE CUTOVER, AND ONE BEHAVIOUR THAT
// DELIBERATELY DID NOT.
//
// modelToAPIFirewallGroup, setResourceData and firewallGroupToModel are gone;
// the descriptor's Fields do that work. Their assertions are re-expressed here
// against the descriptor rather than deleted.
//
// ONE CASE CHANGED ON PURPOSE. The old firewallGroupToModel nulled `members`
// when the API returned none, and a test named "empty members produces null
// set" asserted it. members is Required in the generated schema, so a null
// state against a config holding a value is an inconsistent-result-after-apply
// -- and `members = []` is a legal configuration, so the trigger is reachable.
// The descriptor keeps the empty set. That is a user-visible change and this
// test now asserts the new behaviour, with the old one named so the change is
