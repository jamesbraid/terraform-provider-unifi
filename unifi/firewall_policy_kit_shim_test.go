...
// derivation for a group reference, the connection_states round-trip. Rewriting
// those assertions to fit new call sites would be rewriting the evidence.
//
...
func modelToFirewallPolicy(
	...
	// sets it in BeforeSend, which the field loop above does not run.
...
// normaliseEndpointLists gives every unset list a string element type.
//
// THE ADAPTER NEEDS THIS AND THE ORIGINAL DID NOT, which is worth saying
// because it is the adapter being different rather than the code. The
// hand-written mapper read model fields directly, so a test model that left
// web_domains at its zero value passed a zero types.List and IsNull() answered
// true. Going through types.ObjectValueFrom instead requires a typed value: a
// zero list reports its element type as missing and the conversion is refused.
// Normalising here keeps the tests exercising what they always did.
