...
func (r *settingResource) Schema(
	...
	//
	// RE-SET BY HAND because a generated schema cannot carry it. Left at the
	// zero value Terraform reads the resource as version 0, never runs the
	// upgrader, and prior state silently stops being migrated.
	resp.Schema.Version = 1
	// LOAD-BEARING FOR THE UPGRADE PATH, not only the schema. UpgradeState
	// derives its prior type from this schema and decodes twelve conntrack
	// durations through it; without the graft the Terraform type differs and
	// prior-version state stops decoding.
