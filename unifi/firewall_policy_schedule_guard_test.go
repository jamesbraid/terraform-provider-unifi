...
// TestTheScheduleHookKeepsWhatTheControllerHolds is task 200's guard in
// fast-loop, and it exists because until now that defect had none.
//
// The fix shipped with an acceptance test that drives a real apply, so it only
// ran when a campaign ran against a controller -- and #207 measured that the
// campaign's own selector could not name it either. The defect it guards is
// silent by construction: schedule had no schema attribute, so terraform showed
// no diff, and an apply that changed only a description turned a rule running
// 09:00-17:00 into one running permanently.
//
// IT IS GUARDABLE HERE ONLY BECAUSE THE MIGRATION MADE IT SO. The hand-written
// Update did the read inline against a live client, with no seam a unit test
// could reach. firewallPolicyCarrySchedule takes the read as a parameter, so
// the whole behaviour is exercisable with a function literal and no controller
// -- which is why this was recorded with a trigger rather than written earlier.
func TestTheScheduleHookKeepsWhatTheControllerHolds(t *testing.T) {
	...
	t.Run("a declared schedule is left alone", func(t *testing.T) {
		// THE NEWEST CLAUSE AND THE ONE WHOSE LOSS IS WORST. ToSDK runs first,
		// so a practitioner's block is already encoded; carrying the
	...
	t.Run("an update carries the controller's own schedule", func(t *testing.T) {
		...
		if sdk.Schedule == nil || sdk.Schedule.Mode != "EVERY_DAY" {
			t.Fatalf("an update sent %+v, not the controller's EVERY_DAY schedule.\n"+
				"    This is task 200: a rule the practitioner scheduled for business hours "+
				"runs permanently after an apply that changed something else, and terraform "+
	...
	t.Run("the site falls back to the provider's", func(t *testing.T) {
		...
		// errors, the literal stands, and the schedule is reset with nothing to
		// show for it. So the site the read is CALLED with is asserted, not
		// just the outcome.
		var sawSite string
		sdk := &ui.FirewallPolicy{}
		hook := firewallPolicyCarrySchedule(
