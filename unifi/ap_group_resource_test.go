...
func Test_apGroupKit_readPath(t *testing.T) {
	...
	// DELIBERATE CHANGE FROM THE HAND-WRITTEN MAPPER, recorded rather than
	// dropped. That mapper turned an empty name into a NULL. name is Required,
	// and a Required attribute may not be null in state -- Terraform rejects the
	// result before the value is ever read. The kit's own elide rule says the
	// same thing: Required means KeepZero. The controller does not return
	// nameless groups, so neither behaviour is reachable in practice; this one
...
// Test_apGroupKit_neverWritesForWLANConf is #184, now asserted as a property of
// the descriptor rather than of a hand-kept list.
//
	...
	for _, name := range spec.WireNames() {
		if name == "for_wlanconf" {
			t.Fatal("for_wlanconf is a declared field, so it goes on the wire and " +
				"a whole-object write would reset it on every apply (#184)")
		}
	}
