...
func Test_unifiProvider_Schema(t *testing.T) {
	...
	// TestProviderSchemaSnapshot walks resource, data source, action and
	// list-resource schemas but never the provider block itself, so this is
	// the only thing that checks these attributes exist. It doesn't give the
	// provider block the same protection every other surface has, though: an
	// attribute that appears here without also being named below wouldn't be
	// caught, the way the schema snapshot catches an unexpected attribute
	// elsewhere.
...
// Test_unifiProvider_EphemeralResources pins the current, deliberate state --
// no ephemeral resources are implemented yet -- so a future accidental
// deletion of a real entry, or an accidental nil return, is caught rather
// than silently passing.
func Test_unifiProvider_EphemeralResources(t *testing.T) {
	p := &unifiProvider{}
	got := p.EphemeralResources(context.Background())
	if got == nil {
		t.Error("EphemeralResources() returned nil, want a non-nil empty slice")
	}
	if len(got) != 0 {
		t.Errorf("EphemeralResources() = %d entries, want 0 (none implemented yet)", len(got))
	}
}
