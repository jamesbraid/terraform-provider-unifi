...
// applies it to every declared port. Measured against a live controller: a
// port that declares only "name" still carries "poe_mode" in the outgoing
// write when another port in the same call declares poe_mode -- at
// poe_mode's Go zero value, an empty string. On that measurement the
// controller's own validator rejected the empty string outright (poe_mode
// is enum-constrained) and refused the whole call; a member with a looser
// validator -- an ordinary string, a bool -- would have been accepted and
// silently overwritten. So ports with different declared member sets can
// never share a call. The grouping key is the *set* of names, not their
// order and not their union: an update touching N distinct member-sets
// costs N calls, not one.
...
// unsafe. Wired into the device resource's update path by
// deviceKitBeforeSend.
