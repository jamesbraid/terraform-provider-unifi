...
// network whose purpose DOES carry them must produce nothing, or a check that
// warned about everything would satisfy every must-warn case below.
...
// ONLY WHAT WAS SET. The whole scoping decision rests on this: an attribute the
// practitioner never mentioned is a zero field here, and warning about it would
// put 44 unactionable warnings on every vlan-only plan.
...
//
// ipsec_pfs on a site-vpn is dropped when false, because the encoder tags it
// omitempty where the struct does not. This helper cannot report that: by the
// time a value reaches the SDK struct, "the practitioner set false" and "the
// practitioner said nothing" are the same zero. Reporting zeroes would put 44
// unactionable warnings back on every plan, so the trade is deliberate -- and
// it is why site_to_site_vpn's descriptor suppresses that write directly
...
func TestAZeroFieldIsNotReportedEvenWhereTheEncoderWouldDropIt(t *testing.T) {
	...
	// pfs=false is a zero field, so it is correctly NOT reported here -- the
	// helper cannot distinguish it from unset. That limit is real and is why
	// the descriptor suppresses the write instead.
