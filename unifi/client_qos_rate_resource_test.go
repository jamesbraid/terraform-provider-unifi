...
// THE MAPPING COVERAGE THAT SURVIVED THE CUTOVER.
//
// planToClientQosRate and clientQosRateToModel are gone; the descriptor's
// Fields do that work now. The behaviour they asserted has not gone anywhere,
// so neither has the assertion -- it is expressed against the descriptor
// instead of against two deleted functions.
//
// ELEVEN OTHER TESTS IN THIS FILE WERE DELETED AND NOTHING WAS LOST. Each was
// already recorded in internal/testaudit/unfailable-tests.txt as empty-table
// or no-assertion: a table literal with no entries, so the loop never ran.
// Their ledger entries go with them, which is the only honest way to remove a
// test -- if the ledger still named them, it would be describing a file that
// no longer has them.
