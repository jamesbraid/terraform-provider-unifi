...
// of mdns's masked-write gate, shaped exactly like
// TestCountryBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey
// (setting_country_descriptor_test.go): it runs mdnsKitBackend's
...
// "key", not custom_services/predefined_services when only mode changed.
func TestMdnsBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey(t *testing.T) {
	...
	backend := mdnsKitBackend(api)
	sdk := &settings.Mdns{
		Mode: "custom",
		CustomServices: []settings.SettingMdnsCustomServices{
			{Address: "_myservice._tcp.local", Name: "my service"},
		},
	}
...
// TestMdnsNestedSchemaHasExactlyItsAttributes guards mdnsNestedSchema's
// type assertion against a generator regression: "mdns" moving off
...
func TestMdnsNestedSchemaHasExactlyItsAttributes(t *testing.T) {
	ctx := context.Background()
	built := resource_setting.SettingResourceSchema(ctx)
	if _, ok := built.Attributes["mdns"]; !ok {
		t.Fatal(`the generated setting schema has no "mdns" attribute`)
	}
	nested := mdnsNestedSchema(ctx)
	if len(nested.Attributes) != 3 {
		t.Errorf("mdns has %d attribute(s), want 3; update mdnsKitSpec and this count together",
