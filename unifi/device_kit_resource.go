...
func (r *deviceKitResource) Schema(
	...
	// removed. See the upgraders in deviceKitSchema.
	//
	// The released schema describes this surface in plain text -- all 118 of
	// its paths -- and a generated schema always carries a MarkdownDescription,
	// which makes description_kind markdown for the block and every attribute.
	// That is practitioner-visible and the baseline is authoritative, so it is
	// stripped rather than licensed as a change. Same call and same reason as
	// ap_group and port_profile.
	plainDescriptions(&resp.Schema)
	resp.Schema.Version = 2
