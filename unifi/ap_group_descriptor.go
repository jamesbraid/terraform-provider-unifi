...
// The ap_group descriptor.
//
// Four attributes and no conditionals, which makes it the smallest surface the
// kit serves -- but the membership set carries two behaviours that a plain
// StringSetField would have dropped, and both are silent when lost.
//
// It also closes #184 BY CONSTRUCTION rather than by a list. The hand-written
// resource sent the whole object, so for_wlanconf went back as false on every
// apply; the fix was a hand-kept mask naming the two fields the resource owns.
// The kit derives that mask from Spec.Fields, so a field with no entry has no
// wire name to name and cannot be written. The mask file is deleted rather
// than rewritten.
...
func apGroupKitSpec() resourcekit.Spec[apGroupKitModel, ui.APGroup] {
	return resourcekit.Spec[apGroupKitModel, ui.APGroup]{
		...
		// NOTHING DECLARES for_wlanconf, and that is the whole of the #184 fix.
		// The mask is derived from the list below, so a field absent from it
		// has no wire name and cannot be sent.
		Fields: []resourcekit.Field[apGroupKitModel, ui.APGroup]{
			...
			resourcekit.StringSetField[apGroupKitModel, ui.APGroup]{
				...
				// AND THE ELEMENT TYPE IS NOT ENOUGH. MACAddressType gives each
				// element semantic equality, but a Set identifies its members by
				// string value, so that never reaches the set. Overwriting a
				// practitioner's "AA-BB-.." with the controller's "aa:bb:.."
				// leaves a diff no apply can settle: device_macs comes back from
				// the config in the original spelling every time.
