...
// The radius_user descriptor.
//
// SMALL, AND NOT THE PLAIN CONTROL IT LOOKED LIKE. Seven attributes, no
// conditionals, no collections -- but the account's VLAN is DERIVED: when vlan
// is not set and network_id is, the account inherits that network's VLAN, which
// means a controller lookup during the write. BeforeSend does it, and the
// lookup is keyed on a model field rather than the site, so it cannot be a
// Prefetch.
//
// The SDK type is Account, not RADIUSUser -- a guess at that name is what
// nearly had this surface reported as blocked on a missing masked update.
...
func radiusUserKitBackend(client *ui.ApiClient) resourcekit.Backend[ui.Account] {
	return resourcekit.Backend[ui.Account]{
		...
		// UpdateAccountFields, where the hand-written resource called
		// UpdateAccount and sent the whole object. Same shape as
		// port_profile's, and one of the sixteen surfaces still doing it.
