...
// The traffic_route descriptor.
//
// THE FIRST SURFACE WHERE A SCATTERED OBJECT WRITES A WIRE FIELD NO MEMBER OF
// IT MAPS TO. `destination` holds domain, ip and region, and the controller
// takes four arrays plus a matching_target discriminator saying which of them
// it should read. Nothing the practitioner writes is called matching_target;
// it follows from WHICH member they filled in. So the object spans five wires,
// and the fifth is a value only the encoder knows.
//
// BOTH OBJECTS ARE ALSO CLAIMS, and the ledger names them: destination.ip is
// ONE released list over TWO observed arrays, split on whether an entry
// contains a hyphen, and source.networks plus source.clients are TWO released
// lists over ONE observed array, partitioned on each entry's own type
// discriminator. Neither shape is a projection of an SDK struct, which is why
// ObjectField cannot express either and ScatteredObjectField can.
//
// source SCATTERS ACROSS EXACTLY ONE WIRE, which reads like the wrong kind
// until you look at the cardinality rather than the width. ObjectListField
// needs a LIST on the model and target_devices is one object; ObjectField
// needs an SDK struct per model object and there is no TrafficRouteSource.
// The kind is about a model object not corresponding to an SDK object -- how
// many wires it lands on is incidental.
...
//
// TWO DEFAULTS ARE SEEDED IN New RATHER THAN DERIVED IN A HOOK, and both are
// values the hand-written whole-object PUT sent on every write. matching_target
// falls back to INTERNET and target_devices to a single ALL_CLIENTS entry, so a
// route naming neither a destination nor a source still says "all traffic, all
// clients" rather than leaving the controller to guess.
//
// SEEDING ALONE WOULD NOT REACH THE CONTROLLER. A masked update sends what the
// mask names, and a null object contributes no names -- ScatteredObjectField
// writes nothing when its model value is null, deliberately, so an absent
// object never zeroes a sibling. Both names therefore also appear in
// AlwaysWire, which is what turns the seed into a value that travels.
//
// network_id IS THE THIRD, and it is a lookup rather than a constant: an unset
// network_id means the site's primary WAN, which only the controller knows.
// BeforeSend fills it from Prefetch's inventory when the encoded object left it
// empty, and AlwaysWire carries it for the same reason -- the attribute is
// Optional+Computed, so a plan that omits it holds an unknown and the field's
// own SetInPlan reports nothing to send.
func trafficRouteKitSpec() resourcekit.Spec[trafficRouteKitModel, ui.TrafficRoute] {
	return resourcekit.Spec[trafficRouteKitModel, ui.TrafficRoute]{
		...
		Fields: []resourcekit.Field[trafficRouteKitModel, ui.TrafficRoute]{
			...
			// domains, regions, ip_addresses and ip_ranges hold the values;
			// matching_target says which one the controller reads. Omitting it
			// would leave a route whose domains changed to an ip list still
			// matching on DOMAIN -- the apply succeeds and the route matches
			// nothing.
			//
			// EACH BRANCH SETS matching_target AND THE LAST ONE WINS, which is
			// the hand-written mapper's behaviour preserved rather than
			// corrected. The schema does not stop a practitioner filling in two
			// members; if they do, ip beats region beats domain because that is
			// the order the branches run in. Changing it here would be a silent
			// semantic change in a migration commit.
			...
			// source SCATTERS ACROSS EXACTLY ONE WIRE, which reads like the
			// wrong kind until you look at the cardinality rather than the
			// width. ObjectListField needs a LIST on the model and
			// target_devices is one object; ObjectField needs an SDK struct per
			// model object and there is no TrafficRouteSource. The kind is
			// about a model object having no SDK object to correspond to -- how
			// many wires it lands on is incidental.
			resourcekit.ScatteredObjectField[trafficRouteKitModel, ui.TrafficRoute]{
...
// operation, and only when it is going to be needed.
//
// THE LOOKUP IS DEFERRED RATHER THAN EAGER. Prefetch runs on every create, read
// and update, and listing every network on a site to satisfy an attribute the
// practitioner usually sets would be a request per apply for nothing. The
// closure returned here does the call on first use, so a route naming its
// network_id never makes it.
...
// trafficRouteBeforeSend fills network_id from the primary WAN when the encoded
// object left it empty.
//
// IT READS THE SDK OBJECT, NOT THE MODEL, and the difference matters on update.
// The attribute is Optional+Computed, so a plan that does not mention it holds
// the prior state value rather than a null -- asking the model whether the
// practitioner "set" it would re-derive a network_id they had pinned. What the
// hook actually needs to know is whether the object about to go on the wire
// carries one, which is exactly what ToSDK just decided.
