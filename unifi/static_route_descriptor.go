...
// The static_route descriptor.
//
// THE SURFACE THAT FOUND THE TWO NEW CAPABILITIES AND IS THE FIRST TO USE THEM.
// next_hop is iptypes.IPAddress rather than types.String, which no field kind
// could bind until StringLikeField existed; and `interface` and `next_hop` are
// each sent only for their own route type, which no accessor could express
// because the SDK accessor sees *S and can decide while the Model accessor
// returns a pointer and cannot.
//
// The constant needed nothing new: New seeds the SDK object, and Type is always
// "static-route" on this surface.
...
//
// The hand-written mapper switched on the model's `type` and assigned
// `interface` or `next_hop` only under the matching case. Nothing else
// suppressed them -- the ConfigValidators check that network and next_hop share
// an IP version, not which field belongs to which route type -- so without this
// the kit would send whatever the plan happened to hold.
...
func staticRouteKitSpec() resourcekit.Spec[staticRouteKitModel, ui.Routing] {
	return resourcekit.Spec[staticRouteKitModel, ui.Routing]{
		...
		// THE CONSTANT LIVES HERE. Every routing object this surface writes is
		// a static route, and the hand-written mapper set it on every send.
		...
		Fields: []resourcekit.Field[staticRouteKitModel, ui.Routing]{
			...
			// NO ELIDE, BECAUSE THE DEFAULT REPLACES IT. The hand-written
			// mapper reported "default" whenever the controller returned an
			// empty gateway_type, so an empty read has a value rather than an
			// absence and there is no zero left for an elision to judge.
			resourcekit.StringField[staticRouteKitModel, ui.Routing]{
				Wire:        "gateway_type",
