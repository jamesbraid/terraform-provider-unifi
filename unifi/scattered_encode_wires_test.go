...
// A SCATTERED ENCODE CAN ASSIGN A FIELD ITS OWN Wires LIST DOES NOT NAME, and
// nothing until now could see it.
//
// ScatteredObjectField is the one kind whose write side is a hand-written
// closure rather than a single accessor. Wires is what reaches the mask; Encode
// is what writes the struct. The two are maintained by hand, separately, and a
// field assigned but not declared is accepted at plan, written into the SDK
// object, and then filtered straight back out by the mask. The practitioner's
// change is silently not applied.
//
// THAT IS NOT HYPOTHETICAL. It is vpn_server's local_port, which reached
// unifi.Network through a helper the mask check never looked inside: the port
// was accepted at plan and never written. That bug was found on a controller,
// not by a test, and this is the test that would have found it.
//
// The other direction -- a name in Wires that is not a real json tag -- is
// already covered by WireNameProblems.
...
// assignedSDKFields collects `sdk.Field = ...` targets, including through the
// helper calls an Encode delegates to.
//
// ONE LEVEL, and only helpers named in this package, for the reason the older
// mask parser gives: a transitive crawl would follow into go-unifi and start
// reporting fields no encoder touches.
...
// A BeforeSend HOOK CAN ASSIGN A FIELD NO WIRE NAME DECLARES, which is the same
// defect one level up.
//
// The hook is where a surface derives what no Field can express -- network's
// purpose, vlan and networkgroup, device's port_overrides. Whatever it writes
// still has to reach the mask, and the mask is built from Fields plus
// AlwaysWire. A hook that assigns something neither declares produces an object
// carrying the right value and a mask that filters it out.
//
// This is the half of TestNetworkManagedWireFieldsMatchTheResource that
// survives the move to the kit. That test parsed modelToNetwork and compared it
// against a hand-maintained list; the list is now derived from the descriptor,
// so only the hook needs checking.
