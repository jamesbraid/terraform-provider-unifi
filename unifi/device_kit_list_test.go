...
// A LISTED DEVICE WITH NO OVERRIDES MUST STILL SAY WHAT ITS OVERRIDES ARE A
// SET OF. The list path starts from a zero model -- there is no prior state
// to inherit a type from -- and port_override is not a Field, so nothing on
// that path gave the null its element type. The framework then refused the
// whole list result: "types.SetType[!!! MISSING TYPE !!!]", measured on a
// live controller by TestAccDeviceList_basic.
