...
// The policy file and the surfaces on the kit must agree in BOTH directions.
//
// WHY BOTH. A missing closure does not compile; missing data just is not there.
// A check that only asks "does every policy entry name a real surface" lets a
// surface drop out of generation with everything green, and a check that only
// asks "does every surface have an entry" lets the file accumulate entries for
// surfaces that no longer exist. Each of those is a version of the same failure
// this lane keeps finding -- an outcome that reads as complete while doing no
// work on the part at issue -- and one direction cannot see the other's.
//
// The failure names the surface, because "the policy is out of sync" sends the
...
func kitServedSurfaces(t *testing.T) map[string]string {
	...
	if len(names) != len(embeds) {
		t.Errorf("%d type(s) embed the kit but %d declare a Metadata TypeName; a kit-served "+
			"surface with no Metadata in this package is invisible to schemabehaviour",
