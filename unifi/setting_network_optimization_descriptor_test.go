...
// TestNetworkOptimizationBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey is
// the unit half of network_optimization's masked-write gate, shaped exactly
// like TestMgmtBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey
// (setting_mgmt_descriptor_test.go): it runs
// networkOptimizationKitBackend's UpdateFields closure -- the same one
// Configure wires into the live resource -- against an httptest server that
// keeps the raw, undecoded PUT body, and asserts it carries exactly the
// field the mask named plus "key".
func TestNetworkOptimizationBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey(t *testing.T) {
	...
	backend := networkOptimizationKitBackend(api)
	sdk := &settings.NetworkOptimization{Enabled: true}
