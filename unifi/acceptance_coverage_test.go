...
// coverageIndex is the acceptance corpus read as a call graph rather than as a
// pile of files.
//
// A per-function scan of string literals undercounts badly, because most
// configs come from a helper: testAccAPGroupFrameworkConfig_basic("...") is the
// whole Config field, and the HCL lives one call away. Every surface whose
// tests use a helper would read as uncovered.
...
//
// THE TWO ARE DIFFERENT AND THE DIFFERENCE IS THE FINDING. A test that seeds a
// network in order to attach a firewall policy to it creates a network, so a
// count of "surfaces appearing in acceptance HCL" credits unifi_network for it.
// Nothing in that test asserts a single network attribute, so a network that
// came back wrong would still pass. Coverage that only creates is coverage
// against a panic, not against a defect.
//
// So two sets are collected per surface: what a test declares, and what a test
// checks. The ranking is over the second.
func TestRankTheAcceptanceCoverageGap(t *testing.T) {
	...
	// CEILING CHECK. Every surface a test names must be one the provider
	// actually serves. A name here that is not served is either a typo in the
	// HCL -- which terraform would reject at plan time, but only if the test
	// ever ran -- or a surface that was removed and left behind in a config.
	...
	// CONTROLS. Each names a specific fact, so a walk that silently stopped
	// reaching the corpus fails here rather than reporting a clean sweep.
	//
	// The indirection control is the load-bearing one: ap_group's config is
	// only ever produced by a helper, so it can only be found through a call.
...
// TestNoAcceptanceStepExpectsAnyError is the gap the coverage census does not
// find, because the surfaces it names DO have acceptance tests.
//
// ExpectError makes a step's success conditional on the apply FAILING. With a
// pattern of ".*" the condition is satisfied by any message at all, so the step
// passes on every possible failure and fails only when everything works. That
// inverts the test.
//
// AND IT SWALLOWS THE CHECKS TOO, which is the part that is not obvious from
// reading the step. testStepNewConfig runs step.Check and returns its failure as
// an error (testing_new_config.go:238, `return fmt.Errorf("Check failed: %w")`),
// and testing_new.go:465 then matches that error against the pattern. A ".*"
// pattern matches "Check failed: ..." as happily as it matches a controller
// rejection. TestAccDNSRecordFramework_basic is the live instance: four
// TestCheckResourceAttr calls sit in the step, one of them asserts name is
// "test-record" while the config sets "test-record.example.com", and the test
// passes anyway.
//
