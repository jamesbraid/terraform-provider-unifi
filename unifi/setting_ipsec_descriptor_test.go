...
// TestIpsecBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey is the unit half
// of ipsec's masked-write gate, shaped exactly like
// TestCountryBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey
// (setting_country_descriptor_test.go): it runs ipsecKitBackend's
...
func TestIpsecBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey(t *testing.T) {
	...
	backend := ipsecKitBackend(api)
	sdk := &settings.Ipsec{Ikev2ReauthenticationMethod: "make-before-break"}
	if _, err := backend.UpdateFields(
		context.Background(), "default", sdk, "ikev2_reauthentication_method",
	); err != nil {
		t.Fatalf("UpdateFields: %v", err)
	}

	want := map[string]bool{"key": true, "ikev2_reauthentication_method": true}
...
// TestIpsecKitSpecConformance runs the same conformance instruments every
// other kit descriptor's test applies (see setting_mgmt_descriptor_test.go's
// TestMgmtKitSpecConformance), scoped to ipsec's own nested schema rather
// than a whole resource's, since ipsec is one section of unifi_setting
// rather than a surface of its own.
func TestIpsecKitSpecConformance(t *testing.T) {
...
// TestIpsecNestedSchemaHasExactlyItsAttributes guards ipsecNestedSchema's
// type assertion against a generator regression: "ipsec" moving off
...
func TestIpsecNestedSchemaHasExactlyItsAttributes(t *testing.T) {
	ctx := context.Background()
	built := resource_setting.SettingResourceSchema(ctx)
	if _, ok := built.Attributes["ipsec"]; !ok {
		t.Fatal(`the generated setting schema has no "ipsec" attribute`)
	}
	nested := ipsecNestedSchema(ctx)
	if len(nested.Attributes) != 1 {
		t.Errorf("ipsec has %d attribute(s), want 1; update ipsecKitSpec and this count together",
