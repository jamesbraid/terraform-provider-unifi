...
// TestUswBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey is the unit half
// of usw's masked-write gate, shaped exactly like
// TestCountryBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey
// (setting_country_descriptor_test.go): it runs uswKitBackend's
...
func TestUswBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey(t *testing.T) {
	...
	backend := uswKitBackend(api)
	sdk := &settings.Usw{DHCPSnoop: true}
...
// TestUswKitSpecConformance runs the same conformance instruments every
// other kit descriptor's test applies (see setting_mgmt_descriptor_test.go's
// TestMgmtKitSpecConformance), scoped to usw's own nested schema rather
// than a whole resource's, since usw is one section of unifi_setting
// rather than a surface of its own.
func TestUswKitSpecConformance(t *testing.T) {
...
// TestUswNestedSchemaHasExactlyItsAttributes guards uswNestedSchema's
// type assertion against a generator regression: "usw" moving off
...
func TestUswNestedSchemaHasExactlyItsAttributes(t *testing.T) {
	ctx := context.Background()
	built := resource_setting.SettingResourceSchema(ctx)
	if _, ok := built.Attributes["usw"]; !ok {
		t.Fatal(`the generated setting schema has no "usw" attribute`)
	}
	nested := uswNestedSchema(ctx)
	if len(nested.Attributes) != 1 {
		t.Errorf("usw has %d attribute(s), want 1; update uswKitSpec and this count together",
