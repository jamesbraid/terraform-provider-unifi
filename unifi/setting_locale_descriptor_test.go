...
// TestLocaleBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey is the unit half
// of locale's masked-write gate, shaped exactly like
// TestCountryBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey
// (setting_country_descriptor_test.go): it runs localeKitBackend's
...
func TestLocaleBackendUpdateFieldsSendsOnlyTheNamedWiresPlusKey(t *testing.T) {
	...
	backend := localeKitBackend(api)
	sdk := &settings.Locale{Timezone: "America/Los_Angeles"}
	if _, err := backend.UpdateFields(context.Background(), "default", sdk, "timezone"); err != nil {
		t.Fatalf("UpdateFields: %v", err)
	}

	want := map[string]bool{"key": true, "timezone": true}
...
// TestLocaleKitSpecConformance runs the same conformance instruments every
// other kit descriptor's test applies (see setting_mgmt_descriptor_test.go's
// TestMgmtKitSpecConformance), scoped to locale's own nested schema rather
// than a whole resource's, since locale is one section of unifi_setting
// rather than a surface of its own.
func TestLocaleKitSpecConformance(t *testing.T) {
...
// TestLocaleNestedSchemaHasExactlyItsAttributes guards localeNestedSchema's
// type assertion against a generator regression: "locale" moving off
...
func TestLocaleNestedSchemaHasExactlyItsAttributes(t *testing.T) {
	ctx := context.Background()
	built := resource_setting.SettingResourceSchema(ctx)
	if _, ok := built.Attributes["locale"]; !ok {
		t.Fatal(`the generated setting schema has no "locale" attribute`)
	}
	nested := localeNestedSchema(ctx)
	if len(nested.Attributes) != 1 {
		t.Errorf("locale has %d attribute(s), want 1; update localeKitSpec and this count together",
