...
// TestEachDHCPFlagWithItsOperand turns task 196's dhcp_server floor into a
// total, and the reason it was a floor is the finding.
//
// The earlier sweep set each boolean on its own and kept whatever the
// controller accepted. Three were refused, and the refusals named exactly what
// was missing:
//
//	dhcpd_boot_enabled     BootFilenameInvalid
//	dhcpd_ntp_enabled      NtpAddressInvalid
//	dhcpd_gateway_enabled  (absent from both lists -- neither established nor refused)
//
// Those were recorded as unmeasurable. They are not: a flag that switches a
// feature on needs the feature configured, and the controller had already said
// which field. This is the same shape as ip_aliases needing a CIDR rather than
// a bare address, and match_opposite_ips needing a target list to invert. Three
// refusals in three vocabularies, all saying "you set the switch and not the
// thing it switches".
//
// So every flag here is set TOGETHER WITH ITS OPERAND. What the controller then
// holds is the real at-risk set, and what an unrelated apply loses is the real
// count -- rather than a lower bound produced by a fixture that could not turn
// the features on.
func TestEachDHCPFlagWithItsOperand(t *testing.T) {
	...
	flags := []struct {
		...
		// THE THREE THE EARLIER SWEEP COULD NOT ESTABLISH.
		...
		if len(lost) > 0 {
			return fmt.Errorf(
				"an apply whose only change was the vlan reset %d of %d established DHCP "+
					...
					"    EXPECTED TO FAIL UNTIL TASK 196'S dhcp_server HALF IS FIXED.",
