...
// Test_radiusProfileKit_writePath carries over what
// Test_radiusProfileResource_modelToRadiusProfile asserted about the
// hand-written mapper: the scalars reach the SDK and the server blocks arrive
// as elements rather than being dropped.
...
// Test_radiusProfileKit_readPath carries over
// Test_radiusProfileResource_radiusProfileToModel.
func Test_radiusProfileKit_readPath(t *testing.T) {
	...
	// An absent list stays null rather than becoming an empty block, which is
	// what NullZero on the field means: the practitioner declared no auth_server
	// block and state should say so.
...
// Test_radiusProfileKit_planOverridesStatePerField replaces the applyPlanToState
// test. The kit's ApplyPlanToState copies field by field from Spec.Fields, so
// what can go wrong on THIS surface is a field missing from the descriptor: it
// would silently stop following the plan while every other field kept working.
...
// Test_radiusProfileKit_neverWritesTLSEnabled is ae8a994d's property, asserted
// against the derivation rather than a hand-kept list.
//
// tls_enabled is force-emitted by go-unifi and the provider does not model it,
// so a whole-object write reset it on every apply. The kit derives its mask from
// Spec.Fields, so a field with no entry has no wire name to name.
