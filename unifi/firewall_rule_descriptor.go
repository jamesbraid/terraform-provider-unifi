...
// The firewall_rule descriptor.
//
// THE WIDEST SURFACE CUT OVER SO FAR at 30 mapped attributes, and the first to
// use a capability that another surface produced: src_network_type and
// dst_network_type both report "NETv4" on an empty read, which is what
// static_route's gateway_type needed ReadDefault for. Nothing here is new.
//
// Both mappers were entirely mechanical -- no field's value depended on
// another's, so no write predicate appears below.
...
func firewallRuleKitSpec() resourcekit.Spec[firewallRuleKitModel, ui.FirewallRule] {
	...
	return resourcekit.Spec[firewallRuleKitModel, ui.FirewallRule]{
		...
		Fields: []resourcekit.Field[firewallRuleKitModel, ui.FirewallRule]{
			...
			// iptypes.IPAddress on static_route; hwtypes.MACAddress needs no
			// change to it, which is the first evidence the type parameter is
			// a capability rather than one surface's shape.
			...
			// OPTIONAL+COMPUTED AND STILL NullZero, because its schema says
