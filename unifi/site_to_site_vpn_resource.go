...
func (r *siteToSiteVPNKitResource) Schema(
	...
	//
	// It survived the cutover only because TestBuiltSchemaMatchesReleasedBaseline
	// caught its absence. Nothing else would have: the attribute has no field
	// in the descriptor -- a write-only value never enters state -- so no
	// mapping, no Elide and no wire name refers to it.
