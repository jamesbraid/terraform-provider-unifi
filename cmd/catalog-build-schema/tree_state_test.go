...
func TestRefusesWithoutATreeState(t *testing.T) {
	// This command takes its options as a struct rather than as arguments, so
	// the empty value IS the "every flag omitted" case and -output has no
	// earlier check to trip.
	err := run(options{})
	if err == nil {
		t.Fatal("catalog-build-schema ran without a tree state")
	}
