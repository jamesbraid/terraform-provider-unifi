...
// oneFamily renders the declarations the generator emits for a single nested
// object type, in the order and shape it emits them. `extra` is appended to the
// value struct so two families can be made to differ.
...
// Test_anExactDuplicateIsRemoved is the case wan actually produces: the
// generator names a nested object type from the attribute name, so two
// attributes called `options` yield two identical OptionsType declarations.
...
// Test_aDivergentDuplicateIsRefused is the entire safety argument.
//
// If two same-named declarations ever stop matching, deleting either one
// silently discards a real difference -- and NOTHING downstream could notice,
// because the survivor still compiles. So the pass must refuse, and must name
// what it refused on.
...
// Test_aFileWithNoDuplicateIsUntouched is what the other fifty-three generated
// surfaces are. This pass runs over the whole chain, so being inert where it is
// not needed is a property worth pinning rather than assuming.
