...
// A custom type without its import is Go that does not compile. The import is
// collected from the specification rather than hard-coded, because a hard-coded
// one makes the SECOND custom type a compile error in generated code instead of
// a rendered line.
...
func TestActionRefusals(t *testing.T) {
	for name, testCase := range map[string]struct{ spec, want string }{
		// The failure this emitter exists to make impossible: a custom type is
		// the validation on its attribute, and one rendered without its import
		// does not compile. Naming the attribute beats naming the package.
		...
		// A collection or nested attribute reaches here with no scalar member
		// set. Rendering it as whichever member happened to be checked first
		// would produce a schema nobody asked for.
...
func TestActionEmissionIsDeterministic(t *testing.T) {
	first := emit(t, portSpec)
	for range 5 {
		if again := emit(t, portSpec); again != first {
