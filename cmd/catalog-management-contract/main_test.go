...
func TestRefusesWithoutATreeState(t *testing.T) {
	var stderr bytes.Buffer
	code := run([]string{"-admission", "/dev/null", "-build-schema", "/dev/null", "-policy", "/dev/null", "-output", "/dev/null"}, &stderr)
	if code == 0 {
		t.Fatalf("catalog-management-contract ran without a tree state; exit = 0")
	}
