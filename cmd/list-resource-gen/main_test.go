...
// A description that reaches the specification must reach the Go. It is the one
// thing an emitter can drop while producing output that compiles, serves the
// right shape and passes every structural comparison.
...
// The emitted file has to be gofmt-clean as written, because it is committed.
// format.Source runs inside the tool for exactly this reason; if it were
// removed, the only symptom would be a formatting diff in someone's next commit.
...
// The refusals. Each one exists because the alternative -- emitting something
// plausible -- produces Go that compiles and a schema that is wrong in a way no
// downstream comparison names.
func TestRefusals(t *testing.T) {
	for name, testCase := range map[string]struct {
		...
		// A document with no listresources member is the likeliest mistake in
		// the pipeline: pointing this tool at a managed resource's
		// specification. Writing an empty file would leave an undefined symbol
		// at the call site, which reads as a missing generate step rather than
		// as the wrong input.
...
// Emitting twice from the same document must produce the same bytes. The
// specification carries attributes as a list and the emitter builds Go maps from
// them; ordering that followed Go's map iteration would produce a file that
// differs run to run and a diff in every regeneration.
func TestEmissionIsDeterministic(t *testing.T) {
	first := emit(t, canonicalSpec)
	for range 5 {
		if again := emit(t, canonicalSpec); again != first {
