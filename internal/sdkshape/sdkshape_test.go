...
// These cases were each established by hand against the SDK source before this
// package existed, and they exist to judge the tool rather than the SDK. Three
// earlier text-matching passes disagreed with one another and one of them
// contradicted a fact already proven with comm; that contradiction is the only
// reason it was not believed. Keeping the proven cases as fixtures makes the
// same disagreement fail here instead of shipping.
func TestBestReproducesHandVerifiedClassifications(t *testing.T) {
	...
	tests := map[string]struct {
		...
		// Proven with comm: all eleven members are present in the SDK struct,
		// which carries four more the provider omits.
		...
		// The counterpart, and the case an earlier pass attributed to the
		// wrong one of the two because it took an arbitrary first match.
		// source and destination have IDENTICAL member sets, verified against
		// the released schema, and both SDK structs carry fifteen members. So
		// member matching cannot tell them apart even in principle, and only
		// the field each hangs off can. Origin is asserted rather than Best
		...
		// Proven line by line from the resource: these three members are
		// grouped out of flat SDK fields, PfwdInterface, DestinationIP and
		// DstPort, so no struct carries them together.
...
func TestReachabilityExcludesUnrelatedStructs(t *testing.T) {
	...
	// An earlier pass matched port_forward's forward{ip,port} against
	// RADIUSProfileAcctServers purely because the member names coincided.
