...
type Package struct {
	...
	// wrong for any resource whose name differs from its type, and it was
...
// Operates reports the struct an SDK client method reads or writes.
func (p *Package) Operates(method string) (string, bool) {
	carried, ok := p.operates[method]
	return carried, ok
}
...
// Best returns the reachable struct that best accounts for want.
//
// Selection is deterministic — fewest missing members, then fewest extras, then
// name — because an arbitrary choice among equally good candidates produces
// attributions that swap between runs, which is exactly what an earlier
// text-matching pass did with FirewallPolicySource and FirewallPolicyDestination.
