...
// IMPORT BY NAME IS A FEATURE THE CUTOVER DROPPED, and the drop was invisible
// until a live controller: network's documented handle is "name=Test VLAN" and
// wlan's is the bare SSID, both resolved by the hand-written resources and
// neither parsed by the kit. The kit routed the whole handle into id, the
// backend read found nothing, and the framework turned the clean not-found
// into "Missing Resource Identity After Read" -- because an import that never
// set an identity has none to fall back on.
//
// The routing rule is the one both old resources shared: an explicit "name="
// prefix is a name, a 24-hex handle is an id, anything else is a name. A
// surface opts in by supplying Backend.ReadByName and Spec.Name; without them
// the kit routes every handle to id exactly as before.

func namedKitResource(readByName func(context.Context, string, string) (*kitSDK, error)) *Resource[kitModel, kitSDK] {
	// The feature's opt-in is BOTH members: routing without a resolver would
	// strand the handle on an attribute nothing reads. Tests that only
	// exercise the routing get a stub resolver.
	if readByName == nil {
...
func TestReadReportsANameThatResolvesToNothing(t *testing.T) {
	...
	// The name path is only reachable right after a name import -- a created
	// resource always has its id -- so a name that resolves to nothing is a
	// failed import, not a deletion to record silently.
	if !resp.Diagnostics.HasError() {
