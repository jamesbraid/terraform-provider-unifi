...
// THE OMITEMPTY SUFFIX IS NOT PART OF THE NAME. A check comparing the raw tag
// would report every optional field on every surface, which is the kind of
// noise that gets a check deleted rather than read.
...
// A field reaching something with no json tag is REPORTED, not skipped. Nothing
// names it on the wire, so a mask cannot carry it and the write silently does
// nothing -- the same outcome as a wrong name, reached a different way.
