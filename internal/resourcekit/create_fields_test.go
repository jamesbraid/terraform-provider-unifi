...
// UnwritableWires SUBTRACTS, and the kit does the subtracting so that a hook
// which over-reports fails loudly rather than quietly masking a subset.
//
// The warrant for any particular surface's hook belongs with that surface --
// unifi_network's is in unifi/network_unwritable_wires_test.go, and it is the
// one that proves a dropped name is genuinely never emitted rather than merely
// absent at its zero. These two cover the mechanism: what it removes, and what
// it does when it removes everything.
func TestUnwritableWiresIsSubtractedFromTheMask(t *testing.T) {
	...
	// unwritable, so the mask empties and the write must be refused rather than
	// sent saying nothing.
	if !resp.Diagnostics.HasError() {
