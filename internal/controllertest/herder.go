...
func StartDevices(
	...
	//
	// Running the binary named by envHerderBin is this fixture's purpose, so
	// the command being a variable is the contract rather than a weakness:
	// the value comes from whoever started the test run, who could run the
	// same binary directly. Every argument is built here from the fixed fleet
