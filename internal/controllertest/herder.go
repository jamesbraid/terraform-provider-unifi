...
// unifi-emu-herder is a foreground process that starts fake UniFi devices
// as Docker containers on a network the caller already owns: this harness
// owns the network, the controller, adoption and the assertions, while the
// herder owns only device planning and container lifecycle. Its supported
// integration surface is the versioned NDJSON protocol on stdout; stderr is
// diagnostics and device logs, never parsed here.
//
// The two herder coordinates share go-unifi's names on purpose, since both
// repositories drive the same process against the same controller images.
const (
...
func StartDevices(
	...
	// The child is not tied to ctx: it must outlive a cancelled setup context
	// so its own SIGTERM cleanup still removes the containers. The command
	// itself is a variable by design -- it runs whatever binary envHerderBin
	// names, which the operator already controls -- and every argument comes
	// from the fixed fleet or the controller's own inspected network, never
	// from test data.
	cmd := exec.Command(bin, args...) // #nosec G204 G702 -- see above: the operator-named binary is the contract
