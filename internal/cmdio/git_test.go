...
// separately from GitOutput.
//
// `git status --porcelain` emits "XY filename" where a leading space is a valid
// status value. An unstaged modification is " M tracked.txt". TrimSpace turns
// that into "M tracked.txt", which reads as staged-modified -- a different state,
// on the first line, which is the line a tree-state check looks at.
...
// TestFailureCarriesWhatGitSaid is the whole reason stderr is captured. Eight of
// the nine original copies reported an exit status and discarded the message,
// which is the defect this project has lost the most time to.
...
// TestEmptyRepoRunsInTheCurrentDirectory covers the two original callers that
// passed no repository at all.
